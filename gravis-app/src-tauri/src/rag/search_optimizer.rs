...
pub fn apply_intelligent_boost(
    ...
    // Boost basique par alignement
    let alignment_boost = match (query_intent, document_category) {
        (QueryIntent::Business, DocumentCategory::Business) => 0.15,
        (QueryIntent::Academic, DocumentCategory::Academic) => 0.12,
        (QueryIntent::Legal, DocumentCategory::Mixed) => 0.10,
        (QueryIntent::Technical, DocumentCategory::Academic) => 0.08,
        _ => 0.0,
    };
    ...
    // Type-aware capping final
    type_aware_cap(query_intent, document_category, boosted_score)
}
...
impl SearchEngine {
    ...
    pub fn search_with_optimization(
        ...
        for (i, (doc_id, content, _embedding, category)) in raw_results.iter().enumerate() {
            ...
            // Score hybride avec recette par intent (sur scores normalisés)
            let hybrid_score = match query_intent {
                QueryIntent::Business => 0.4 * cosine_norm + 0.6 * bm25_norm,
                QueryIntent::Academic => 0.55 * cosine_norm + 0.45 * bm25_norm,
                QueryIntent::Legal => 0.35 * cosine_norm + 0.65 * bm25_norm,
                QueryIntent::Technical => 0.5 * cosine_norm + 0.5 * bm25_norm,
                QueryIntent::General => 0.5 * cosine_norm + 0.5 * bm25_norm,
            };
