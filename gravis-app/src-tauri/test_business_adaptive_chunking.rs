...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📄 Content length: {} chars", business_content.len());
    ...
    let academic_chunks = academic_chunker.chunk_document(
        ...
        SourceType::NativeText,
        &ExtractionMethod::DirectRead,
        "test_academic_comparison"
    )?;
    
    println!("Academic chunker on business content:");
    println!("  📖 Chunks: {}, Sections: {}, Avg size: {:.1}", 
             academic_chunks.chunks.len(), 
             academic_chunks.sections_detected.len(),
             academic_chunks.avg_chunk_size);
             
    println!("Business chunker on business content:");
    println!("  💼 Chunks: {}, Sections: {}, Avg size: {:.1}", 
             chunks.chunks.len(), 
             chunks.sections_detected.len(),
             chunks.avg_chunk_size);

    // Le chunker business devrait détecter plus de sections pertinentes (si des chunks sont créés)
    if chunks.chunks.len() > 0 && academic_chunks.chunks.len() > 0 {
        // Comparaison seulement si les deux fonctionnent
        println!("📊 Comparison possible - both chunkers produced results");
    } else {
        println!("⚠️  Cannot compare - one or both chunkers failed to produce chunks");
    }
    ...
    let doc_category = classifier.classify(business_content)?;
